...
public class UserShopCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sub.equalsIgnoreCase("설정")) {
            ...
            p.sendMessage(color("&7(관리자 설정 명령은 기존 그대로 유지)"));
            return true;
        }
    ...
    private void sendHelp(Player p) {
        ...
        if (isAdmin) {
            ...
            p.sendMessage(color("&c[관리자] /유저상점 설정 ... (기존과 동일)"));
        }
    }
}
    // fallback: '/유저상점 열기' -> open player's personal shop
    private void _fallbackOpen(org.bukkit.entity.Player p) {
        try {
            try { com.minkang.ultimate.usershop.gui.GUIManager.openMyShop(p); return; } catch (Throwable ignore) {}
            try { com.minkang.ultimate.usershop.gui.GUIManager.open(p); return; } catch (Throwable ignore) {}
            p.performCommand("상점");

...
public class UserShopCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if ("등록".equals(args[0])) {
            ...
            // Need register ticket
            ItemStack ticket = ItemSerializer.deserializeFromBase64(plugin.getConfig().getString("items.register-ticket", ""));
            if (ticket == null || !ItemUtils.consumeOne(p, ticket)) {
                p.sendMessage(plugin.msg("need-register-ticket"));
                return true;
            }
            ItemStack inHand = p.getInventory().getItemInMainHand();
            if (inHand == null || inHand.getType().name().equals("AIR")) {
                p.sendMessage(plugin.msg("no-item-in-hand"));
                ItemUtils.giveItem(p, ticket); // rollback ticket
                return true;
            }
            if (inHand.getAmount() < amount) {
                p.sendMessage(plugin.msg("not-enough-item"));
                ItemUtils.giveItem(p, ticket); // rollback ticket
                return true;
            }
            PlayerShop shop = sm.getOrCreateShop(p.getUniqueId());
            int max = sm.getMaxSlotsFor(p.getUniqueId());
            if (slot < 0 || slot >= max) {
                p.sendMessage(plugin.msg("invalid-slot"));
                ItemUtils.giveItem(p, ticket); // rollback
                return true;
            }

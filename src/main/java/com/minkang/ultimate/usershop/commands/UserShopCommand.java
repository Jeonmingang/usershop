...
public class UserShopCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sub.equalsIgnoreCase("추가")) {
            ...
            // 등록권: 인벤토리 어디든 1개 소모 (이름/로어 완전 일치)
            ...
            p.getInventory().setItemInMainHand(null); // 손 아이템 제거
        ...
        if (sub.equalsIgnoreCase("확장")) {
            ...
            if (!inHand) {
                p.sendMessage(color("&c손에 확장권을 들고 있어야 합니다."));
                return true;
            }
        ...
        if (sub.equalsIgnoreCase("설정")) {
            if (!p.hasPermission("usershop.admin")) {
                p.sendMessage(color("&c권한이 없습니다."));
                return true;
            }
            if (args.length < 2) {
                p.sendMessage(color("&c사용법: /유저상점 설정 <오픈|등록권|확장권|확장삭제|제거> ..."));
                return true;
            }
            String action = args[1];
            if (action.equalsIgnoreCase("오픈")) {
                ItemStack hand = p.getInventory().getItemInMainHand();
                if (!ItemUtil.applyTemplateFromItem(plugin, "opener-item", hand)) {
                    p.sendMessage(color("&c손에 든 아이템이 비어있습니다."));
                } else {
                    plugin.saveConfig();
                    p.sendMessage(color("&a오픈 아이템이 설정되었습니다. (이름/로어 완전 일치)"));
                }
                return true;
            } else if (action.equalsIgnoreCase("등록권")) {
                ItemStack hand = p.getInventory().getItemInMainHand();
                if (!ItemUtil.applyTemplateFromItem(plugin, "tickets.register", hand)) {
                    p.sendMessage(color("&c손에 든 아이템이 비어있습니다."));
                } else {
                    plugin.saveConfig();
                ...
                ItemStack hand = p.getInventory().getItemInMainHand();
                if (!ItemUtil.applyTemplateFromItem(plugin, "tickets.expand", hand)) {
                    p.sendMessage(color("&c손에 든 아이템이 비어있습니다."));
                } else {
                    plugin.saveConfig();
                ...
                if (args.length < 4) {
                    p.sendMessage(color("&c사용법: /유저상점 설정 확장삭제 <플레이어> <횟수(1~6)>"));
                    return true;
                }
                String targetName = args[2];
                int times;
                try { times = Integer.parseInt(args[3]); } catch (Exception e) {
                    p.sendMessage(color("&c횟수는 숫자여야 합니다."));
                    return true;
                }
                int maxSteps = plugin.getConfig().getInt("defaults.max-expansions", 5) + 1; // 0~5 = 6단계
                if (times < 1 || times > maxSteps) {
                    p.sendMessage(color("&c횟수는 1~" + maxSteps + " 사이여야 합니다."));
                    return true;
                }
                OfflinePlayer target = p.getServer().getOfflinePlayer(targetName);
                if (target == null || target.getName() == null) {
                    p.sendMessage(color("&c플레이어를 찾을 수 없습니다."));
                    return true;
                }
                ...
                if (args.length == 3) {
                    int slot;
                    try { slot = Integer.parseInt(args[2]); } catch (Exception e) {
                        p.sendMessage(color("&c슬롯은 숫자여야 합니다."));
                        return true;
                    }
                    ...
                    if (target == null) {
                        p.sendMessage(color("&c온라인 플레이어만 지원합니다."));
                        return true;
                    }
                    int slot;
                    try { slot = Integer.parseInt(args[3]); } catch (Exception e) {
                        p.sendMessage(color("&c슬롯은 숫자여야 합니다."));
                        return true;
                    }
                ...
                p.sendMessage(color("&c알 수 없는 설정입니다."));
                return true;
            }
        }

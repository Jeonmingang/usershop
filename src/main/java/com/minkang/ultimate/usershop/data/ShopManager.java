...
public class ShopManager {
    ...
    public Collection<PlayerShop> allShops() {
        return shops.values();
    }
    ...
    public int getMaxSlotsFor(UUID uuid) {
        return getSlotCount(uuid);
    }
    ...
    public void unregisterListing(OfflinePlayer player, int slot, boolean refundItem, boolean refundTicket) {
        ...
        if (listing != null) {
            ...
            if (player.isOnline()) {
                ...
                // refund one register ticket, if configured
                if (refundTicket) {
    ...
    public void handlePurchase(Player buyer, UUID sellerId, int slot, int amount) {
        ...
        if (!ok) {
            // refund money if full inventory

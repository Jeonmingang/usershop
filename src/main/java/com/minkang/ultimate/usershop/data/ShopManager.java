...
public class ShopManager {
    ...
    public ShopManager(Main plugin) {
        ...
        if (!usersFile.exists()) try { usersFile.createNewFile(); } catch (IOException ignored) {}
        usersCfg = YamlConfiguration.loadConfiguration(usersFile);
    }

    public void saveAll() { try { usersCfg.save(usersFile); } catch (IOException ignored) {} }
    ...
    public boolean removeListingToPlayer(UUID uuid, int slot, Player receiver) {
        ...
        if (stack != null) {
            ...
            if (!left.isEmpty()) for (ItemStack s : left.values()) receiver.getWorld().dropItemNaturally(receiver.getLocation(), s);
        }
        return true;
    }

    public boolean adminRemoveListingToPlayer(UUID uuid, int slot, Player receiver) { return removeListingToPlayer(uuid, slot, receiver); }

    public static class MarketEntry { public UUID owner; public int slot; public double pricePerUnit; public ItemStack item; public String ownerName;
    ...
    public static class OwnerEntry { public UUID owner; public String ownerName; public int itemCount; public double minPrice;
    ...
    public List<OwnerEntry> getOwners() {
        ...
        for (String uuidStr : playersSec.getKeys(false)) {
            ...
            if (count > 0) { if (min == Double.MAX_VALUE) min = 0.0; out.add(new OwnerEntry(uuid, name, count, min)); }
        }

...
// --- Added helpers for title check & refresh ---
public boolean isOurInventoryTitle(String title) {
    String s = org.bukkit.ChatColor.stripColor(title == null ? "" : title);
    return s.contains("유저상점") || s.contains("검색 결과") || s.contains("상점");
}
public void clearView(org.bukkit.entity.Player p) { /* optional: if you track views, clear here */ }
public void refresh(org.bukkit.entity.Player p) {
    // optional: reopen current view; if you track context, call openMain/openOwnerShop/openSearch
    openMain(p, 1);
}

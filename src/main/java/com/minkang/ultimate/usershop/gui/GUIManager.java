...
public class GUIManager {
    ...
    private String getPdc(ItemMeta im, String key) {
        try {
            NamespacedKey k = new NamespacedKey(plugin, key);
            PersistentDataContainer pdc = im.getPersistentDataContainer();
    ...
    public void openSearch(Player p, int page, String rawQuery) {
        ...
        for (MarketEntry me : list) {
            String dname = (me.item.hasItemMeta() && me.item.getItemMeta().hasDisplayName())
                    ? ChatColor.stripColor(me.item.getItemMeta().getDisplayName()).toLowerCase()
                    : me.item.getType().name().toLowerCase();
            boolean ok = dname.contains(rawQuery.toLowerCase());
            if (!ok) {
                for (String q : norm) {
                    if (dname.contains(q)) { ok = true; break; }
                }
            }
    ...
    public void handleContentClick(Player p, ItemStack clicked) {
        ...
        if (vc.mode == Mode.MAIN) {
            String ownerStr = getPdc(im, "usershop-owner");
            if (ownerStr == null) return;
            try { openOwnerShop(p, java.util.UUID.fromString(ownerStr), 1); } catch (Exception ignored) {}
        }
    }
